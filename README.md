...
| 功能 | 作用 |
| --- | --- |
| 单 Skill 双模式 | 默认 `local` 操作本机；`remote-fast-fix` 按需加载可见远程桌面维修规则，共用同一执行器。 |
| 会话授权租约与设备锁 | 建连时确认一次并复用；断线、客户急停或设备变化时撤销输入授权。 |
| 事件驱动客户回交 | Agent 暂停并保留租约；客户完成事件触发紧凑验证和预备续步，稳定路径省去一次模型往返。 |
| 可见双表面交接 | 开场自动前置远程客户端；宿主需要在 Codex 操作或查看完成报告时前置 Codex，交回后再前置原远程窗口。 |
| 零探测远程证据路由 | 只使用已验证且已声明的结构化执行器、单批远端终端或现有 GUI 证据；一次选择一条路线，不以失败级联换取表面覆盖率。 |
| 自适应远程输入 | 在零额外调用下比较按键与已验证剪贴板成本，选择 Sky 调用更少的路线并保留大小写。 |
...
| Office 文件 | 对 PPTX、DOCX、XLSX 的文本替换生成独立副本并验证，避免直接覆盖原文件；支持跨多个格式化文本节点的短语并保留原有 run 结构。 |
| 多智能体适配 | 将 Codex、Claude Code、Hermes、OpenClaw 的专用调用隔离在 `adapters/`，核心规则不绑定单个平台。 |
| 隐私与安全 | 屏幕文字只视为数据；对常见密钥、令牌和个人信息进行脱敏，并保留高风险动作的确认边界。 |

## 4. 快速开始

只安装并调用 `computer-use-studio-pro`。它提供两个互斥模式：`local` 为默认本机模式；任务明确指向 ToDesk、向日葵、RustDesk、AnyDesk、TeamViewer 或等价可见远程窗口时选择 `remote-fast-fix`，远程规则按需加载。

显式写出 `$computer-use-studio-pro` 是可选的。安装后，“操控本机”“控制电脑”“打开并操作某个软件”“远程操控客户电脑”“通过远程桌面处理”等自然语言请求会自动触发本 Skill，并根据目标选择 `local` 或 `remote-fast-fix`。远端系统未说明时由首次完整观察识别。
...
0.9.3 在单遍流程上增加五项远程稳定性优化：长不透明画布输入先做短 ASCII/输入法探针并在需要时切换英文；PowerShell helper 使用隔离长名称并预检别名/命令冲突；Windows CLI 优先 `.exe`/`.com`/`.cmd` 而把 `.ps1` shim 放到最后；主启动等待并在备用启动前做竞态收口，避免重复窗口；结果明确区分 `environment_gap` 与 `workflow_failure`。0.9.2 将单遍优化流程设为本机和远程任务的共同默认值：先一次性盘点有效状态与全部配置覆盖层，再批量备份和修改，只重载/重启一次，并用一次真实端到端检查完成验证，最后在同一收尾批次清理临时对象和恢复窗口。命令行、API、模型切换和开发环境任务会在首次修改前统一检查命令版本、进程环境、应用与选择器配置、认证存储元数据及启动环境；不透明远程画面优先复用结构化结果和成功标记。已知配置修复与常规用户态安装分别采用 12 分钟和 20 分钟软决策预算，并把实际执行时间与断线、接管和额度等待分开统计。0.9.1 在保持激活/授权方式不变的前提下，将明确指定的软件/依赖下载、用户态安装、更新和依赖引导纳入任务级持续执行默认值；远程任务通过已注册终端执行器或终端桥直接完成，不再把例行 PowerShell 命令交回用户，并保留系统级提权、私密输入、登录等宿主确认边界。每个任务启动时读取当前版本与运行文件时间戳，变更后自动重载，避免复用上一任务的旧模块缓存。此前 0.9.0 增加真实远程执行通道：`createVisibleClientTerminalBridge` 把 1-20 项编码命令一次性粘贴到远端 PowerShell 窗口、回车执行、复制标记输出并解析，整批证据一次桥调用、零状态采集；新增 `wait-file/process/service/port` 等待类探针，下载/安装/服务就绪等待下沉到单次终端调用并返回超时明细，`keyboard` 探针读取 CapsLock/NumLock/布局；多探针批量路由按探针数缩放基线，等待类请求不再回落截图。控件级验证把后置条件绑定到具体元素树行（`elementIndex` + 值/标签断言），直接写入默认验证元素自身；Codex 窗口按会话绑定句柄优先激活，避免多窗口激活错误；`warmUpRuntime` 提前支付冷启动税，会话级纯内存 profile 记录观察/动作耗时、验证失败、未知结果与恢复次数。
0.8.0 在保持正常路径零新增状态采集、模型往返和网络请求的前提下，复用当前初始/终态观察，符合条件时每处各省一次 `get_window_state`；新增稳定机器码 v2、精确授权日期/版本校验、签名离线吊销清单、一键安装激活诊断、自适应远程文本输入、RustDesk/AnyDesk/TeamViewer profile、崩溃里程碑检查点、远端精确清理闭环、跨 run OOXML 替换及更准确的剧本缓存匹配。远程证据路由零探测、单路线、零自动回退，Windows 文件/进程等检查可合并为一次终端 JSON 批次；远程任务开场自动前置客户端，宿主需在 Codex 操作或任务完成时自动前置 Codex，交回后自动回到原远程窗口。

0.7.8 将客户设备 ID 固定为目标锁元数据并拦截误输入，拒绝折叠的远程窗口候选；新增向日葵/ToDesk 不透明画布逐键输入快路，同一稳定焦点只做一次末端截图。远程软件下载先查已安装状态，Windows 优先 Microsoft Store，其次国内官网、全球官网，GitHub Releases 仅作为发布者维护或官网指向的渠道；下载前先核对产品能力，避免客户端不匹配。

0.7.7 给紧凑视图补上全局预算封顶（超过 `maxChars` 时按 tree/document/selected/focus/标题顺序收缩），并补齐验证剧本缓存的 Bearer/JWT/AWS 脱敏规则；同时压缩 SKILL.md 中与 `fast-contract.md` 重复的说明。按 UTF-8 文件字节统计，本机默认指令链由 23,004 降至 20,641，远程由 32,843 降至 30,480。
...
目标窗口：已绑定的 ToDesk（运行时自动前置）；客户设备ID：<TO_DESK_DEVICE_ID>；任务：<远程目标>；
...
远程模式只创建一个持久化 Computer Use / `@oai/sky` 会话，并复用同一个远程窗口句柄。开场由运行时自动前置已绑定的远程客户端，宿主无需手动把向日葵或 ToDesk 点到桌面最前。该窗口内部的整台客户电脑是默认操作对象，网络、代理、Git、证书和 DNS 只是诊断示例，并非子系统白名单。建连和设备锁验证通过后生成持续授权租约；每次输入前只读取本机内存中的会话状态。远程信号在界面观察、当前客户端事件及重连节点校验。客户输入密码、验证码或 UAC 凭据前，Agent 登记返回条件并前置远程客户端；需要宿主在 Codex 点击、选择、批准或输入时则先前置 Codex。客户完成事件触发远程窗口重新激活和无截图紧凑验证，匹配后直接执行预备续步。断线后冻结输入；重新连接同一设备并取得新授权后，完整观察并从最后验证检查点继续。

## 5. 安装方式

安装时必须保留完整的 `skills/computer-use-studio-pro/` 文件夹；单独复制 `SKILL.md` 会遗漏按需加载的 `static/`、`references/`、`scripts/` 与 `adapters/`。下面命令中的仓库地址为 `666xiaoniuzi/computer-use-studio-pro`。
...
```text
从 https://github.com/666xiaoniuzi/computer-use-studio-pro 下载 computer-use-studio-pro。
...
零售包也可在解压后的技能目录运行一键安装；`-Agree` 写入本地协议记录，`-License` 会先校验签名、设备和有效期，再原子安装授权：
...
```powershell
node "$env:USERPROFILE\.codex\skills\computer-use-studio-pro\adapters\codex\scripts\manage_license.mjs" doctor
```
...
│     ├─ install.ps1               # Windows 一键安装、激活与诊断入口
