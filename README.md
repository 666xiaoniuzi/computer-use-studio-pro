...
| 命令行＋GUI 混合验收 | 本地零调用选择执行面：确定性工作走结构化/终端，交互工作走界面，桌面软件用快速设置加一次真实用户流程验收。 |
...
0.9.4 新增零调用的结构化/终端/GUI/混合路由和 `presence → launch → functional → user_flow` 分级验收；Windows 执行锁定选中的绝对启动器，远程命令批次默认走免输入法影响的已验证剪贴板桥。终端批次使用每次唯一标记排除旧剪贴板结果，并只在传输层声明长度限制或截断时分块，普通路径仍是一写一贴且不增加截图、模型往返、网络请求或 Token 视图。0.9.3 增加输入法预检、PowerShell helper 冲突隔离、原生/CMD 启动器排序、启动去重和结构化结果分类。0.9.2 将一次盘点、批量修改、单次重载、真实验收和收尾恢复设为共同默认流程。
