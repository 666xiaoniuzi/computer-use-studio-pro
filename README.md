...
0.7.3 增加交付物语义命名、最终 Token 使用报告和宿主机可见回交。新文件在创建前从任务目标或内部标题确定最终名称，保存后核对确切文件名；远程任务结束后最小化/关闭远程窗口并显示宿主机桌面；计量优先采用宿主精确 Token，缺少时显示由既有紧凑视图生成且明确标注的估算值，全程不新增观察或模型往返。
