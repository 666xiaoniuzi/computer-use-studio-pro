...
5. Verify with the cheapest reliable evidence and keep only the smallest useful output. In remote mode, routine/action refreshes collect text plus a runtime-retained screenshot in one state call; semantic changes never auto-trigger a second capture. Use an explicit semantic-only route for bounded polls and customer fast return.
