...
From already-declared capabilities, call `selectTaskInteractionRoute` once. Deterministic setup/inspection uses structured or terminal execution; interactive work uses GUI; a desktop product normally combines fast execution with GUI acceptance. Route selection itself performs no probe, capture, network request, or model turn.
...
For Windows command work, build and execute the absolute-path invocation selected by `buildWindowsCliInvocation`; do not redispatch the bare name after ranking native/CMD launchers ahead of `.ps1`. Create collision-resistant helpers after a `Get-Command` preflight. An absent dependency is `environment_gap`, not execution-path failure.
...
5. Declare required evidence as `presence`, `launch`, `functional`, or `user_flow`; lower evidence never completes a higher requirement. Use the cheapest reliable evidence and smallest output. Structured results, exit codes, logs, and markers prove nonvisual work; desktop acceptance exercises the real GUI once. Reuse the same terminal observation, and keep semantic polls/customer return screenshot-free.
...
For a remote disconnect, call `cusproUsage.pauseActive("disconnect")`, revoke the current authorization, and retain the last verified checkpoint. Reconnect to the same device, obtain fresh authorization, capture a complete view, reconcile committed effects, call `cusproUsage.resumeActive()`, and continue from the first unmet postcondition. A device switch or emergency stop latches the session in `stopped`.

For a customer-entered private value, call `cusproUsage.pauseActive("customer_takeover")` and pause Agent input while retaining the lease. Pre-register the expected return state and optional reversible continuation. Foreground the remote-client window for customer-computer input; foreground Codex before asking the host to click, approve, choose, or type there. After a matching explicit completion event, debounce briefly, reactivate and cheaply check the bound remote window, call `cusproUsage.resumeActive()`, take one compact screenshot-free observation, and continue locally only on the expected state. A mismatch returns to G3; a disconnect or target-lock event follows normal recovery.

## G4-CLOSE

Confirm terminal evidence, including the exact semantic name and location of every deliverable. After `success_verified=true`, distill semantic prechecks, steps, postconditions, and rollback into the verified playbook cache in the same closeout call; omit secrets, identities, paths, screenshots, coordinates, handles, and expiring indexes. For remote mode, clean and verify exact task-created nonessential artifacts on the remote side, end Agent input and the task lease, disconnect when the contract calls for it, then clean and verify the local controller side. Minimize or close the remote-client window and foreground the host task surface before reporting; Codex activates the current Codex task window. For remote mode, report the result, material side effects, both cleanup states, unresolved takeover points, Token usage, total wall-clock duration, active execution duration, and categorized excluded waits. Include start/finish timestamps and `HH:MM:SS.mmm`; prefer exact host input/output/cache counts, otherwise report the in-memory meter's labelled compact-view estimate and basis. Ordinary chat and local completion omit the usage/timing block unless duration is requested. Capture the final meter report once after handback and reuse collected metrics so closeout adds no observation or model roundtrip. Never turn an attempted action into a completed result.
