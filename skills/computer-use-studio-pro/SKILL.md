...
On Windows, resolve CLI candidates with native/CMD launchers before PowerShell shims (`.exe`/`.com`/`.cmd`/`.bat` before `.ps1`). Give every PowerShell batch helper a collision-resistant `__Cusp_*` name and run a case-insensitive command/alias preflight before defining it. After a primary app/terminal launch, wait for its window and make one final list check before invoking a fallback. Record missing commands or dependencies as `environment_gap`; reserve `workflow_failure` for an executed automation path that failed.
