...
- For ToDesk or Sunlogin, create one `createRemoteClientSignalAdapter` in the persistent Codex runtime and pass its synchronous `connectionVerifier`, `deviceVerifier`, and `stopSignalVerifier` to the session. This keeps routine connection/device classification local, retains the initial device baseline while its ID is temporarily hidden, and latches a newly visible conflicting device ID without another model decision.
...
- Treat coordinate, focus, and semantic references as expiring observation leases. Persistent sessions enforce shorter coordinate leases and longer focus/semantic leases. On `STALE_OBSERVATION_LEASE`, refresh the required surface, remap the target, and then issue a new input rather than replaying an old coordinate or element index.
