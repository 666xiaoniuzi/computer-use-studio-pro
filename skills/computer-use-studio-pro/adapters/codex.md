...
- Use the shared single-pass default: one read-only effective-state/override inventory, one backed-up idempotent mutation batch, one reload or restart, one real functional test, and one cleanup/window-restore close batch.
...
Call `selectTaskInteractionRoute(...)` once with already-known capabilities. Structured/terminal routes execute deterministic installation, configuration, diagnostics, and file work; GUI handles interactive workflows; desktop products use fast execution plus a real GUI acceptance check. The selector performs zero calls. For software acquisition, run one read-only override inventory, one backed-up idempotent mutation batch, one reload/restart, and one functional test without routine prompts or terminal handback. Prefer non-interactive per-user installation; pause only at a host boundary.
...
Submitted terminal commands set `commandBatch: true` and use the verified clipboard/terminal bridge, avoiding IME dependence. A key-event command fallback needs `imePreflight` or an `asciiVerification` proof bound to the current `layoutEpoch`; the session caches that proof. Use `buildWindowsCliInvocation(...)` and execute its absolute-path command so later resolution does not return to a `.ps1` shim. Use `launchOnceAndAwaitWindow(...)` before fallback. Outcomes declare `requiredLevel`/`achievedLevel` (`presence`, `launch`, `functional`, `user_flow`); a lower level becomes `unknown`, while environment gaps remain distinct from workflow failures.

When a PowerShell/Windows terminal is available, import `remote_evidence.mjs`, create `createVisibleClientTerminalBridge(...)`, and pass it to `createRemoteEvidenceRouter`. One bridge call pastes and runs 1-20 probes, copies uniquely marked output, parses it, and restores the clipboard with zero state captures/model turns. Unique per-batch markers reject stale copied output. A declared clipboard limit splits before paste; a truncation receipt triggers adaptive chunks before Enter, leaving the normal one-paste path unchanged. Use bounded `wait-*` and `keyboard` probes instead of GUI polling. Keep the verified terminal focused and use its confirmed select/copy/paste keys.
