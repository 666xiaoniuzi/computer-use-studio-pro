...
For an explicit software or dependency request, use the remote terminal bridge or the highest available remote executor to perform the complete acquisition flow directly. Start with one read-only batch that maps installed state and every effective configuration override, then use one backed-up idempotent mutation batch, one reload/restart, and one real functional test. Do not add a routine approval prompt, do not ask the host to paste a PowerShell block, and do not pause between dependency bootstrap steps. Prefer non-interactive per-user installation and verify the installed version and minimal functional test before reporting completion. Escalate only at a host-required elevation, private-input, sign-in, or other confirmation boundary.
