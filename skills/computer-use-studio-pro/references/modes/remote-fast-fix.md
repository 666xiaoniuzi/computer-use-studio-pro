...
6. Prefer current evidence. Select execution/acceptance locally from declared capabilities: structured/terminal for deterministic work, GUI for interactive work, hybrid for desktop products. Then use one call-free evidence route. On an opaque canvas, submitted commands prefer the verified clipboard bridge; ordinary fields use the lower-call transport.
...
2. Call `selectTaskInteractionRoute` from known capabilities, then run one short read-only feasibility batch: OS, installed state, actual capability, dependency path, required evidence level, and override layers. For command/API/model tools include command/version, process environment, app/selector config, auth metadata, and startup environment without secret values. Classify fault and keep `environment_gap` separate from `workflow_failure`.
...
5. Reload/restart once and satisfy the declared `presence`, `launch`, `functional`, or `user_flow` level. Structured output proves nonvisual work; a desktop product receives one real GUI flow. Reuse current evidence and pivot on the second unchanged attempt.
...
- Before typing, activate and focus the target. Submitted commands set `commandBatch: true` and prefer the verified clipboard/terminal bridge, bypassing IME state. Key-event command fallback requires an epoch-bound `asciiVerification`; ordinary fields retain lower-call selection. Selection adds zero calls.
- On Windows, execute `buildWindowsCliInvocation(...).command` so the selected `.exe`/`.com`/`.cmd`/`.bat` absolute path cannot resolve back to `.ps1`. Use collision-resistant helpers and one race-closing list check before fallback.
...
- Use `waitUntil` only when no terminal bridge exists. The single-batch bridge uses unique markers to reject stale clipboard output and locally splits only on a declared limit or truncation receipt; the ordinary one-paste path is unchanged. Keep waits inside `wait-*` probes and mark known opaque-canvas transitions.
