...
2. **One window and device lease:** bind the current ToDesk/Sunlogin window by app, returned handle, stable title substring, and the exact customer device ID. An additional session identity cue remains useful. Validate the window and device lock on every observation and immediately before every input.
...
10. **Target lock:** before each input, confirm the current window still belongs to the bound remote client and the observed customer device ID exactly matches the lock. A window, application, title, identity, or device-ID mismatch latches `stopped`, revokes authorization, and ends input for that session.
11. **Continuous authorization:** activate authorization only after the initial connected state and device lock pass. Keep it active for that connected session. A customer emergency stop or device-lock violation latches `stopped`; a disconnect moves to `disconnected`. Each transition revokes input authorization immediately.
...
For Codex, use `createPersistentWindowSession` from `adapters/codex/scripts/sky_fast_path.mjs`. Supply `mode`, `window`, `targetApp`, `targetTitleIncludes`, `remoteDeviceId`, `taskScope`, `success`, and an authorization signal. Wire customer stop to `emergencyStop`, out-of-band disconnect to `markDisconnected`, and same-device reconnection to `resumeAfterReconnect(..., { reauthorize: true })`. Add a device extractor/verifier, connection verifier, stop-signal verifier, identity cue, or target verifier when available.

The session state is one of `connected`, `stalled`, `disconnected`, `connected-unauthorized`, `rebinding`, or `stopped`. Only `connected` with `authorization_status=active` permits input. `stopped` is latched; continue by creating a new explicitly authorized session. `disconnected` may resume after same-device verification and fresh authorization.
