...
23. when the target device exposes a PowerShell/Windows terminal, run 1-20 read-only probes as one visible-client bridge call (`createVisibleClientTerminalBridge`: paste -> Enter -> copy markers -> parse -> restore clipboard) instead of one state capture per check.
...
## 0.9.0 visible-client bridge and control-scoped verification
