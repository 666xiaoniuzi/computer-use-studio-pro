...
Version 0.9.3 adds five remote-stability improvements to the single-pass flow: a short ASCII/IME probe before long opaque-canvas input, isolated PowerShell helper names with command-collision preflight, native/CMD Windows launchers ahead of PS1 shims, a race-closing wait before any fallback launch, and structured separation of `environment_gap` from `workflow_failure`. Version 0.9.2 makes a single-pass optimized flow the shared default for local and remote tasks: inventory effective state and every override layer once, back up and mutate in one batch, reload or restart once, run one real end-to-end check, then clean temporary artifacts and restore the visible window state in the close batch. CLI, API, model-switching, and development-environment tasks map command versions, process environment, application and selector configuration, auth-store metadata, and startup environment before the first edit. Opaque remote canvases reuse structured results and success markers. Soft decision budgets are 12 minutes for a known configuration repair and 20 minutes for a routine fresh user-space install, with active execution measured separately from disconnect, takeover, and host-limit waits. Version 0.9.1 keeps the activation/authorization flow unchanged and makes explicit software/dependency download, user-space installation, updates, and dependency bootstrap part of the task-wide autonomous default. Remote tasks use the registered terminal executor or terminal bridge directly instead of handing routine PowerShell blocks back to the user; system elevation, private input, and sign-in remain host confirmation boundaries. Each task start reads the active version and runtime-file timestamp and reloads changed modules instead of reusing an earlier task cache. Version 0.9.0 added the real remote execution channel: `createVisibleClientTerminalBridge` pastes an encoded 1-20 probe batch into the remote PowerShell window, runs it, copies and parses the marker-delimited output — one bridge call, zero state captures. New `wait-file/process/service/port` probes move download/install/ready waits into a single terminal call with timeout details, and the `keyboard` probe reads CapsLock/NumLock/layout. Multi-probe batch routing scales its baseline with the probe count, and wait-only requests never fall back to screenshots. Control-level verification binds postconditions to a specific accessibility tree line (`elementIndex` plus value/label assertions), and direct fill defaults to verifying the element's own value. The session-bound Codex window id is preferred during foreground selection, `warmUpRuntime` pays the cold-start tax before the first decision, and an in-memory session profile records observation/action durations, verification failures, unknown outcomes, and recoveries.
