...
| CLI + GUI hybrid acceptance | Selects execution locally with zero calls: deterministic work uses structured/terminal routes, interaction uses GUI, and desktop products combine fast setup with one real user-flow check. |
...
Version 0.9.4 adds zero-call structured/terminal/GUI/hybrid routing and graded `presence → launch → functional → user_flow` acceptance. Windows execution locks the selected absolute launcher, and submitted remote command batches prefer the verified clipboard bridge to avoid IME dependence. Every terminal batch has unique markers to reject stale clipboard output; chunking activates only when the transport declares a limit or truncation, so the normal path stays one write/paste with no extra capture, model turn, network request, or Token view. Version 0.9.3 added IME preflight, isolated PowerShell helpers, native/CMD launcher ranking, deduplicated launch, and structured outcome classes. Version 0.9.2 established one inventory, batch mutation, single reload, real acceptance check, and cleanup/restore as the shared default.
